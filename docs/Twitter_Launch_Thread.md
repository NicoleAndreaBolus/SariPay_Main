# 🐦 SariPay — Official Twitter / X 3-Tweet Launch Thread

---

### 🧵 Tweet 1 of 3 (Main Announcement — Attach Banner Image)

```text
🚀 Excited to announce SariPay — the B2B supply chain micro-escrow network built natively on @StellarOrg Soroban smart contracts! 💙🌱

Solving Cash-On-Delivery risks & unlocking working capital for 1.3M+ Sari-Sari stores! 🇵🇭

🌐 Try Live: https://saripay.vercel.app/
#Stellar #Soroban @RiseIn
```
*(Exact Length: 254 / 280 Characters — Attach `saripay_x_official_banner.jpg` when posting!)*

---

### 🧵 Tweet 2 of 3 (Problem & Solution)

```text
1/ 📦 The Problem:
Informal retail supply chains face massive friction. Merchants risk upfront payments for missing stock, while suppliers face unpaid inventory on Cash-on-Delivery (COD).

2/ 🔒 The Solution:
SariPay locks stablecoins in Soroban escrow, releasing payout in <5s via QR scan!
```
*(Exact Length: 271 / 280 Characters)*

---

### 🧵 Tweet 3 of 3 (Technical Highlights & Links)

```text
3/ ⚡ Built on Stellar:
• Sub-cent fees & sub-second settlement
• Instant QR delivery handoff
• Soroban Rust Smart Contracts

📂 Open Source Repo: https://github.com/NicoleAndreaBolus/SariPay_Main
🔍 On-Chain Contract: https://stellar.expert/explorer/testnet/contract/CDCYQTQY5TETNSKHGNCJQXDPEUTDAQY4AONAQQPTBLICTDVAVE3VOPDU
```
*(Exact Length: 279 / 280 Characters)*

---

### 🧵 Tweet 1 (Main Announcement + Attach Image)

🚀 Excited to announce **SariPay** — the B2B supply chain micro-escrow network built natively on @StellarOrg Soroban smart contracts! 💙🌱

We are solving Cash-On-Delivery risks and unlocking working capital for 1.3M+ Sari-Sari stores in the Philippines! 🇵🇭

🌐 Try the Live App: https://saripay.vercel.app/
#Stellar #Soroban #Web3 #BuildOnStellar @RiseIn

---

### 🧵 Tweet 2 (The Problem & Solution)

1/ 📦 **The Problem:** 
Informal retail supply chains face massive friction. Merchants risk upfront payments for missing goods, while suppliers risk non-payment on Cash-on-Delivery (COD).

2/ 🔒 **The Solution:** 
SariPay locks stablecoin payments into a Soroban smart contract escrow upon order placement. Payout is released to the supplier in <5s when the merchant scans the delivery QR code!

---

### 🧵 Tweet 3 (Technical Highlights & Open Source)

3/ ⚡ **Built on Stellar:**
- Sub-cent fees & sub-second settlement
- Fast QR handoff verification
- Soroban Rust Smart Contracts

📂 Open Source GitHub Repo: https://github.com/NicoleAndreaBolus/SariPay_Main
🔍 On-Chain Contract: https://stellar.expert/explorer/testnet/contract/CDCYQTQY5TETNSKHGNCJQXDPEUTDAQY4AONAQQPTBLICTDVAVE3VOPDU

---

### 📷 Banner Asset to Attach:
Image file location: [`docs/screenshots/saripay_x_official_banner.jpg`](file:///c:/Users/kazen/Downloads/SariPay-main/SariPay-main/docs/screenshots/saripay_x_official_banner.jpg)
Live Web Link: [https://saripay.vercel.app/screenshots/saripay_x_official_banner.jpg](https://saripay.vercel.app/screenshots/saripay_x_official_banner.jpg)
